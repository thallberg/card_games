...
type StockPileProps = {
  ...
  onDraw?: () => void;
  disabled?: boolean;
  /** Grön ram när det är spelarens tur (samma som 500) */
  isMyTurn?: boolean;
};
...
/** Talongen – kortbaksida, samma namn och stil som 500. */
      ...
      <button
        ...
        className={`relative block overflow-hidden rounded-md border-2 p-0 min-h-[44px] min-w-[44px] transition-colors active:scale-95 disabled:pointer-events-none disabled:opacity-50 ${CARD_CLASS} ${
          isMyTurn
            ...
            : "border-[var(--border)] hover:border-muted-foreground/60"
        }`}
        aria-label="Plocka från talongen"
      >
        <img
          src="/cardback.png"
          alt=""
